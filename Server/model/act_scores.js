...
const attemptSchema = new mongoose.Schema({
  ...
  // 'FullTest' when this subject score came from a Full ACT Test session, 'Module' for standalone practice
  source: { type: String, enum: ['Module', 'FullTest'], default: 'Module' },
  dateAttempted: { type: Date, default: Date.now },
}, { _id: false });

const actScoreSchema = new mongoose.Schema({
  email:   { type: String, required: true },
  name:    String,
  subject: { type: String, enum: ['English', 'Mathematics', 'Reading', 'Science'], required: true },
  attempts: [attemptSchema],  // latest first, capped at 5
}, { timestamps: true });

// Unique index — one doc per student per subject
actScoreSchema.index({ email: 1, subject: 1 }, { unique: true });

module.exports = mongoose.model('act_score', actScoreSchema, 'act_scores');
