...
// POST /api/gate_da_exam_ai_feedback - AI feedback for a single enriched GATE DA exam attempt
router.post('/gate_da_exam_ai_feedback', async (req, res) => {
  try {
    const { student, attempt, enrichedResults, insights } = req.body;
    ...
    const prompt = `
You are an expert GATE Data Science & AI (GATE DA) tutor reviewing a student's exam attempt. Provide a deeply personalised, constructive analysis.
...
`.trim();
    ...
    console.error('GATE DA exam AI feedback error:', error);
    res.status(500).json({ success: false, message: error.message });
  }
});
