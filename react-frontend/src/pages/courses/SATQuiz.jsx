...
// ─── Section config (section slug → subject name + question count) ─────────────
// subject must match what is stored in the DB — the API normalises "and" ↔ "&"
const SECTION_CONFIG = {
  rw: {
    subject: 'Reading and Writing',  // matches DB value "Reading and Writing"
    label: 'Reading & Writing',       // display label shown in UI
    questionCount: 27,
    timeMin: 32,
  },
  math: {
          ...
          <div className="container">
            <div className="row d-flex justify-content-center text-center">
              <div className="col-lg-8">
                <h1>SAT {label} — Module {moduleNum} Review</h1>
              </div>
            </div>
          </div>
        </div>
...
const SATQuiz = () => {
  ...
  const config = SECTION_CONFIG[section] || SECTION_CONFIG.rw
  const { subject, label, questionCount } = config
  const moduleNum = parseInt(module, 10) || 1

  const style = SUBJECT_STYLE[subject] || SUBJECT_STYLE['Mathematics']
  ...
  const fetchQuestions = async () => {
    try {
      const res = await axios.get(
        `${API_URL}/api/sat_questions?subject=${encodeURIComponent(subject)}`,
  ...
  const handleSubmit = async (forced = false) => {
    ...
    setSaving(true)
    try {
      await axios.post(`${API_URL}/api/sat_scores`, {
        email: u.email,
        name: u.username || u.name || u.email,
        subject,  // "Reading & Writing" or "Mathematics"
  ...
  if (loading) return (
    <div className="d-flex justify-content-center align-items-center" style={{ height: '60vh' }}>
      <div className="text-center">
        ...
        <p className="text-muted">Loading SAT {label} — Module {moduleNum} questions…</p>
      </div>
    </div>
  )
        ...
        <nav className="breadcrumbs">
          <div className="container">
            <ol>
              <li><Link to="/">Home</Link></li>
              <li><Link to="/courses/sat">SAT</Link></li>
              <li className="current">{label} — Module {moduleNum}</li>
            </ol>
          </div>
        </nav>
      </div>
