...
const SUBJECT_STYLE = {
  ...
  'Data Interpretation & Logical Reasoning': { gradient: 'linear-gradient(135deg,#c92a2a,#ff6b6b)', badge: '#c92a2a' },
  'Quantitative Ability':                    { gradient: 'linear-gradient(135deg,#f08c00,#ffd43b)', badge: '#f08c00' },
}
...
// ─── CAT Score Dashboard — shown right after a section quiz / the full test is submitted ─
// results:      { correctAnswers, wrongAnswers, unattempted, score, maxScore, percentage, totalTime, totalQuestions?, responses? }
// sections:      [{ label, result }] — one entry per subject (Sections tab; hidden if only one)
// chapterItems:  flat array from buildChapterItems() — powers the Chapters tab
const CATScoreDashboard = ({
  results, sections = [], chapterItems = [],
  onRetake, saving, dateAttempted,
  heroTitle = 'Test Completed!', retakeLabel = 'Retake', backTo = '/courses/cat', backLabel = 'Back to CAT',
}) => {
  ...
  const accuracy = totalQuestions > 0 ? Math.round((results.correctAnswers / totalQuestions) * 100) : 0
  ...
  return (
    <>
      ...
      <div className="d-inline-flex p-1 rounded-pill bg-light mb-4 shadow-sm flex-wrap">
        {tabs.map(t => (
          <button key={t} onClick={() => setTab(t)} className="btn btn-sm rounded-pill px-4 py-2"
            style={{ background: tab === t ? OVERALL_GRADIENT : 'transparent', color: tab === t ? '#fff' : '#495057', fontWeight: 600, border: 'none' }}>
            {t}
          </button>
        ))}
      </div>
            ...
            <div className="col-md-4">
              <div className="card border-0 shadow-sm h-100" style={{ borderRadius: 14, borderLeft: '4px solid #198754' }}>
                <div className="card-body">
                  ...
                  <div className="text-muted small">+{results.score >= 0 ? results.score : 0} marks</div>
                </div>
              </div>
            </div>
            <div className="col-md-4">
              <div className="card border-0 shadow-sm h-100" style={{ borderRadius: 14, borderLeft: '4px solid #dc3545' }}>
                <div className="card-body">
                  ...
                  <div className="text-muted small">−1 per wrong MCQ</div>
                </div>
              </div>
            </div>
            <div className="col-md-4">
          ...
          <div className="card border-0 shadow-sm" style={{ borderRadius: 14 }}>
            <div className="card-body">
              ...
              <div className="progress mb-2" style={{ height: 10 }}>
                <div className="progress-bar" style={{ width: `${pct}%`, background: scoreColor(pct) }} />
              </div>
                ...
                <div className="card border-0 shadow-sm h-100" style={{ borderRadius: 16, overflow: 'hidden' }}>
                  ...
                  <div className="card-body p-4">
                    <div className="d-flex justify-content-between align-items-center mb-3">
                      ...
                      <span className="badge text-white" style={{ background: style.gradient }}>{p}%</span>
                    </div>
                    ...
                    <div className="row text-center">
                      <div className="col-3">
                        <div className="fw-bold fs-6">{r.score}/{r.maxScore}</div>
                        <div className="text-muted small">Score</div>
                      </div>
                      <div className="col-3">
                        <div className="fw-bold fs-6 text-success">{r.correctAnswers}</div>
                        <div className="text-muted small">Correct</div>
                      </div>
                      <div className="col-3">
                        <div className="fw-bold fs-6 text-danger">{r.wrongAnswers}</div>
                        <div className="text-muted small">Wrong</div>
                      </div>
                      <div className="col-3">
                        <div className="fw-bold fs-6 text-secondary">{r.unattempted ?? 0}</div>
                ...
                <div key={i} className="mb-3">
                  <div className="d-flex justify-content-between align-items-center mb-1">
                    <div>
                      ...
                      {sections.length > 1 && (
                        <span className="badge ms-2 text-white" style={{ background: style.badge || '#6c757d', fontSize: '0.7rem' }}>{c.subject}</span>
                      )}
                    </div>
...
export default CATScoreDashboard
