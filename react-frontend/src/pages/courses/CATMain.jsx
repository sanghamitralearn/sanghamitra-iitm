...
const ACCENT      = '#e8590c'
const ACCENT_DARK = '#8c2f0a'

const SECTION_STYLE = {
  'Verbal Ability & Reading Comprehension': {
    color: '#e8590c', gradient: 'linear-gradient(135deg,#e8590c,#ff922b)', icon: 'bi-chat-square-text-fill', short: 'VARC',
  },
  'Data Interpretation & Logical Reasoning': {
    color: '#c92a2a', gradient: 'linear-gradient(135deg,#c92a2a,#ff6b6b)', icon: 'bi-diagram-3-fill', short: 'DILR',
  },
  'Quantitative Ability': {
    color: '#f08c00', gradient: 'linear-gradient(135deg,#f08c00,#ffd43b)', icon: 'bi-calculator-fill', short: 'QA',
  },
}

// A full-test attempt only ever draws the fixed question set defined by CATFullTest's
// STAGES (66: 24 VARC + 20 DILR + 22 QA) — NOT the whole question bank.
...
const FULL_TEST_TOTAL_MARKS = FULL_TEST_TOTAL_QUESTIONS * 3
...
const CATMain = () => {
  ...
  const fetchPapers = async () => {
    try {
      const res = await axios.get(`${API_URL}/api/cat_papers`, { withCredentials: true })
  ...
  const fetchMyAttempts = async (email) => {
    try {
      const res = await axios.get(`${API_URL}/api/cat_full_scores?email=${encodeURIComponent(email)}`, { withCredentials: true })
  ...
  // Builds a full-test-shaped result by pairing the latest VARC + DILR + QA scores —
  // used when no cat_full_scores record exists yet for this paper.
  const getSyntheticFullAttempt = () => {
    ...
    const v = bySubject(STAGES[0].subject)[0]
    const d = bySubject(STAGES[1].subject)[0]
    const q = bySubject(STAGES[2].subject)[0]
    if (!v || !d || !q) return null
    return {
      testType: 'full',
      score: (v.score || 0) + (d.score || 0) + (q.score || 0),
      maxScore: (v.maxScore || 0) + (d.maxScore || 0) + (q.maxScore || 0),
      correctAnswers: (v.correctAnswers || 0) + (d.correctAnswers || 0) + (q.correctAnswers || 0),
      wrongAnswers: (v.wrongAnswers || 0) + (d.wrongAnswers || 0) + (q.wrongAnswers || 0),
      unattempted: (v.unattempted || 0) + (d.unattempted || 0) + (q.unattempted || 0),
      totalQuestions: (v.totalQuestions || 0) + (d.totalQuestions || 0) + (q.totalQuestions || 0),
      dateAttempted: v.dateAttempted,
      responses: [
        ...(v.responses || []).map(r => ({ ...r, subject: STAGES[0].subject })),
        ...(d.responses || []).map(r => ({ ...r, subject: STAGES[1].subject })),
        ...(q.responses || []).map(r => ({ ...r, subject: STAGES[2].subject })),
      ],
      sectionScores: {
        [STAGES[0].subject]: { score: v.score, maxScore: v.maxScore, correctAnswers: v.correctAnswers, wrongAnswers: v.wrongAnswers, unattempted: v.unattempted, totalQuestions: v.totalQuestions },
        [STAGES[1].subject]: { score: d.score, maxScore: d.maxScore, correctAnswers: d.correctAnswers, wrongAnswers: d.wrongAnswers, unattempted: d.unattempted, totalQuestions: d.totalQuestions },
        [STAGES[2].subject]: { score: q.score, maxScore: q.maxScore, correctAnswers: q.correctAnswers, wrongAnswers: q.wrongAnswers, unattempted: q.unattempted, totalQuestions: q.totalQuestions },
      },
    }
  }
  ...
  const handleStartTest = () => {
    ...
    navigate('/courses/cat/full-test', { state: { paper: selectedPaper.paper, year: selectedPaper.year, paperMeta: selectedPaper } })
  }
    ...
    <main className="main">
          ...
          <div className="container">
            <div className="row d-flex justify-content-center text-center">
              <div className="col-lg-8">
                <h1>CAT</h1>
                <p className="mb-0">
                  Full-length CAT papers with the real section structure. Select a paper to attempt VARC, DILR and
                  Quantitative Ability together and get a detailed analysis.
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="container mb-5 mt-4">
          ...
          <div className="alert d-flex align-items-center gap-2 mb-3" style={{ borderRadius: 12, background: '#fff3cd', border: '1px solid #ffc107' }}>
            ...
            <div className="flex-grow-1">
              ...
              <div className="small text-muted mt-1">
                Make sure the server is running, then <button className="btn btn-sm btn-link p-0" onClick={fetchPapers}>retry</button>.
              </div>
            </div>
          </div>
        )}
        ...
        {papers.length === 0 && !apiError ? (
          <div className="text-center py-5">
            ...
            <p className="text-muted">
              Papers appear automatically once CAT questions are added to the database.<br />
              <small><button className="btn btn-sm btn-link p-0" onClick={fetchPapers}>Retry</button></small>
            </p>
          </div>
        ) : papers.length > 0 && (
          <>
            {sortedKeys.map(yearKey => (
              <div key={yearKey} className="mb-5">
                ...
                {grouped[yearKey].map((p, idx) => {
                  ...
                  const pct = attempt ? Math.round(Math.max(0, attempt.score / attempt.maxScore) * 100) : null
                  ...
                  return (
                    <div key={idx} className="card border-0 shadow-sm mb-3 course-item" style={{ borderRadius: 16, overflow: 'hidden' }}>
                        ...
                        <div className="row align-items-center g-3">
                            ...
                            <div className="d-flex align-items-start gap-3">
                              ...
                              <div>
                                <h5 className="mb-1 fw-bold" style={{ color: ACCENT_DARK, fontSize: '1.05rem' }}>
                                  {p.paper || 'CAT Practice Test'}
                                </h5>
                                <p className="mb-0 text-muted small">CAT Full-Length Paper</p>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="col-lg-3">
                            <div className="row g-2 mb-3">
                              ...
                              <div className="col-6">
                                <div className="p-2 rounded text-center" style={{ background: '#fff3cd', border: '1px solid #ffc107' }}>
                                  <div className="fw-bold fs-5" style={{ color: '#856404' }}>{FULL_TEST_TOTAL_MARKS}</div>
                            ...
                            <div className="progress mb-2" style={{ height: 6, backgroundColor: '#f0f0f0' }}>
                              <div className="progress-bar" style={{ width: `${pct ?? 0}%`, background: progressBarColor }} />
                            </div>

                            <div className="text-center">
                              <span className="d-inline-block px-3 py-1 rounded-pill text-white fw-bold mb-1" style={{
                                background: `linear-gradient(135deg,${ACCENT},${ACCENT_DARK})`,
                              ...
                              {attempt ? (
                                <div>
                                  <small className="text-muted d-block">{pct}% score</small>
                          ...
                          <div className="col-lg-3 d-flex flex-column gap-2 align-items-lg-end">
                            <button className="btn fw-semibold text-white" style={{
                              background: `linear-gradient(135deg,${ACCENT},${ACCENT_DARK})`,
                              border: 'none', borderRadius: 10, padding: '0.55rem 1.4rem', minWidth: 150,
                            }} onClick={() => handleOpenPreTest(p)}>
                              <i className={`bi ${attempt ? 'bi-arrow-clockwise' : 'bi-play-fill'} me-2`} />
                              {attempt ? 'Reattempt' : 'Start Test'}
                            </button>
                            {attempt ? (
                              <Link to="/courses/cat/analysis" state={{ results: { ...attempt, testType: 'full' }, questions: null }}
                                className="btn btn-outline-secondary btn-sm" style={{ borderRadius: 10, minWidth: 150 }}>
                                <i className="bi bi-bar-chart-line me-1" />View Analysis
                              </Link>
                            ) : (
                              <button type="button" className="btn btn-outline-secondary btn-sm" style={{ borderRadius: 10, minWidth: 150 }}
                                disabled title="Attempt this test to view its analysis">
                                <i className="bi bi-bar-chart-line me-1" />View Analysis
                              </button>
                            )}
                          </div>
        ...
        <div className="row g-3 mt-2">
          {[
            { icon: 'bi-plus-circle-fill', color: '#e8590c', label: '+3 / −1',      desc: 'per correct/wrong MCQ (TITA: no penalty)' },
            { icon: 'bi-lock-fill',        color: '#c92a2a', label: 'Fixed order',   desc: 'VARC → DILR → QA, no going back' },
            { icon: 'bi-clock-fill',       color: '#f08c00', label: '2h total',      desc: '40 minutes per section' },
          ].map(item => (
      ...
      {showPreTest && selectedPaper && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}
          ...
          <div style={{ background: '#fff', borderRadius: 20, maxWidth: 560, width: '100%', overflow: 'hidden', boxShadow: '0 24px 64px rgba(0,0,0,0.3)' }}
            ...
            <div style={{ background: `linear-gradient(135deg,${ACCENT},${ACCENT_DARK})`, padding: '22px 28px', color: '#fff' }}>
              <div className="d-flex align-items-center gap-3">
                ...
                <div className="flex-grow-1">
                  <h5 className="mb-0 fw-bold">{selectedPaper.paper || 'CAT Practice Test'}</h5>
                  <div className="mt-1" style={{ opacity: 0.85, fontSize: '0.82rem' }}>CAT Full-Length Paper</div>
                </div>
            ...
            <div className="p-4">
              <div className="row g-2 mb-4">
                {[
                  { icon: 'bi-journal-text',    color: ACCENT,    label: 'Paper',           value: selectedPaper.paper || 'CAT Practice Test' },
                  { icon: 'bi-question-circle', color: '#c92a2a', label: 'Total Questions', value: FULL_TEST_TOTAL_QUESTIONS },
                  { icon: 'bi-trophy',          color: '#f08c00', label: 'Total Marks',     value: FULL_TEST_TOTAL_MARKS },
                  { icon: 'bi-clock',           color: '#28a745', label: 'Duration',        value: '2h (40 min/section)' },
                  { icon: 'bi-check2-circle',   color: '#198754', label: 'Correct MCQ',     value: '+3 marks' },
                  { icon: 'bi-dash-circle',     color: '#dc3545', label: 'Wrong MCQ',       value: '−1 mark' },
                ].map(item => (
                    ...
                    <div key={sec} className="mb-2">
                      <div className="d-flex justify-content-between align-items-center mb-1">
                        <span className="small fw-semibold d-flex align-items-center gap-1">
                          <i className={`bi ${s.icon}`} style={{ color: s.color }} />{s.short}
                        </span>
                        <div className="d-flex gap-2">
                          <span className="badge" style={{ background: `${s.color}18`, color: s.color, border: `1px solid ${s.color}44`, fontSize: '0.68rem' }}>
                            {count} Questions
                          </span>
                          <span className="badge" style={{ background: `${s.color}18`, color: s.color, border: `1px solid ${s.color}44`, fontSize: '0.68rem' }}>
                            {count * 3} Marks
              ...
              <div className="rounded p-3 mb-4" style={{ background: '#fff8e1', border: '1px solid #ffe082' }}>
                ...
                <ul className="mb-0 ps-3" style={{ fontSize: '0.8rem', color: '#555', lineHeight: 1.8 }}>
                  <li>+3 marks for each correct MCQ, −1 for wrong, 0 for unattempted</li>
                  <li>TITA questions: +3 correct, 0 wrong/unattempted — no negative marking</li>
                  <li>Sections run strictly in order (VARC → DILR → QA) — no going back once submitted</li>
                  <li>Do not switch browser tabs — violations are recorded</li>
                </ul>
              </div>

              <div className="d-flex gap-2">
                <button className="btn btn-outline-secondary flex-fill" onClick={handleClosePreTest}>Cancel</button>
