...
const MathQuiz = () => {
  ...
  setLoading(true);
  try {
    const url = `${API_URL}/api/competitive_math_questions?week=${validWeekNum}&email=${encodeURIComponent(userRef.current.email)}&count=25`;
  ...
  const handleSubmit = async (forced = false) => {
    ...
    try {
      ...
      await axios.post(`${API_URL}/api/competitive_math_quiz_attempts`, {
        email:    u.email,
      ...
      <div className="page-title" data-aos="fade" style={{ marginBottom: '2rem' }}>
        <div className="heading"><div className="container">
          <div className="row d-flex justify-content-center text-center">
            <div className="col-lg-8">
              <h1>{displayName}</h1>
              <p className="mb-0">Mathematics Assessment</p>
            </div>
          </div>
        </div></div>
        <nav className="breadcrumbs"><div className="container"><ol>
