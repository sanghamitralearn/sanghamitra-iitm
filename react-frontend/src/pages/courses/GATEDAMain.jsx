...
const GATEDAMain = () => {
  ...
  const fetchPapers = async () => {
    try {
      const res = await axios.get(`${API_URL}/api/gate_da_papers`, { withCredentials: true })
  ...
  const fetchMyAttempts = async (email) => {
    try {
      const res = await axios.get(
        `${API_URL}/api/gate_da_full_scores?email=${encodeURIComponent(email)}`,
  ...
  const handleStartTest = () => {
    ...
    navigate('/courses/gate-da/full-test', {
  ...
  return (
    <main className="main">
          ...
          <div className="container">
            <div className="row d-flex justify-content-center text-center">
              <div className="col-lg-8">
                <h1>GATE DA</h1>
                <p className="mb-0">
                  Full-length GATE Data Science &amp; Artificial Intelligence papers with official marking scheme.
                  Select a paper to attempt all 6 subjects together and get a detailed analysis.
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="container mb-5 mt-4">
        ...
        {papers.length === 0 && !apiError ? (
          <div className="text-center py-5">
            ...
            <p className="text-muted">
              Papers appear automatically once GATE DA questions are added to the database.<br />
              <small>
                <button className="btn btn-sm btn-link p-0" onClick={fetchPapers}>Retry</button>
              </small>
            </p>
                            ...
                            <div className="d-flex align-items-start gap-3">
                              ...
                              <div>
                                <h5 className="mb-1 fw-bold" style={{ color: ACCENT_DARK, fontSize: '1.05rem' }}>
                                  {p.paper || 'GATE DA Practice Test'}
                                </h5>
                                <p className="mb-0 text-muted small">GATE DA Full-Length Paper</p>
                              </div>
                            </div>
                          </div>
        ...
        <div className="row g-3 mt-2">
          {[
            ...
            { icon: 'bi-clock-fill',        color: '#0d6efd', label: '6 subjects',   desc: 'General Aptitude through Artificial Intelligence' },
          ].map(item => (
      ...
      {showPreTest && selectedPaper && (
        ...
        >
          <div
            style={{ background: '#fff', borderRadius: 20, maxWidth: 560, width: '100%', overflow: 'hidden', boxShadow: '0 24px 64px rgba(0,0,0,0.3)', maxHeight: '90vh', overflowY: 'auto' }}
          ...
          >
            ...
            <div style={{ background: `linear-gradient(135deg,${ACCENT},${ACCENT_DARK})`, padding: '22px 28px', color: '#fff' }}>
              <div className="d-flex align-items-center gap-3">
                ...
                <div className="flex-grow-1">
                  <h5 className="mb-0 fw-bold">{selectedPaper.paper || 'GATE DA Practice Test'}</h5>
                  <div className="mt-1" style={{ opacity: 0.85, fontSize: '0.82rem' }}>GATE DA Full-Length Paper</div>
                </div>
            ...
            <div className="p-4">
              ...
              <div className="row g-2 mb-4">
                {[
                  { icon: 'bi-journal-text',    color: ACCENT,    label: 'Paper',           value: selectedPaper.paper || 'GATE DA Practice Test' },
                  { icon: 'bi-question-circle', color: '#6610f2', label: 'Total Questions', value: selectedPaper.totalQuestions ?? 24 },
                  { icon: 'bi-trophy',          color: '#ffc107', label: 'Total Marks',     value: selectedPaper.totalMarks ?? 24 },
                  { icon: 'bi-clock',           color: '#28a745', label: 'Duration',        value: '3h' },
                  { icon: 'bi-check2-circle',   color: '#198754', label: 'Correct Answer',  value: '+1 / +2 marks' },
                  { icon: 'bi-dash-circle',     color: '#dc3545', label: 'Wrong MCQ',       value: '−1/3 or −2/3' },
                ].map(item => (
              ...
              <div className="rounded p-3 mb-4" style={{ background: '#fff8e1', border: '1px solid #ffe082' }}>
                ...
                <ul className="mb-0 ps-3" style={{ fontSize: '0.8rem', color: '#555', lineHeight: 1.8 }}>
                  <li>+1 or +2 marks for each correct answer (per question)</li>
                  <li>−1/3 mark for wrong MCQ answers (1-mark questions) or −2/3 (2-mark questions)</li>
                  <li>No negative marking for Multiple-Select (MSQ) or Numerical Answer Type (NAT) questions</li>
                  <li>You can navigate freely between all sections</li>
