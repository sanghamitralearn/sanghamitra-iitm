...
const Math2Quiz = () => {
  ...
  const fetchQuestions = async () => {
    try {
      const res = await axios.get(`${API_URL}/api/iitm_math2_questions?week=${weekNum}`, { withCredentials: true })
      const data = res.data
  ...
  const handleSubmit = async (forced = false) => {
    ...
    try {
      ...
      await axios.post(`${API_URL}/api/iitm_math2_scores`, {
        email: u.email,
      ...
      <div className="page-title" data-aos="fade" style={{marginBottom:'2rem'}}>
        <div className="heading"><div className="container">
          <div className="row d-flex justify-content-center text-center">
            <div className="col-lg-8">
              <h1>{displayName}</h1>
              <p className="mb-0">IITM Mathematics II — Week {weekNum} Assessment</p>
            </div>
          </div>
        </div></div>
        <nav className="breadcrumbs"><div className="container"><ol>
